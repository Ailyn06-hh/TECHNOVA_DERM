...
export async function GET(req: NextRequest) {
  try {
    // 1. Validar que la computadora esté registrada como dispositivo POS
    ...
    // 2. Obtener cajas de esta sucursal y comprobar turnos actualmente abiertos
    ...
    // Consultar turnos abiertos en la sucursal
    ...
    const cajas = cajasRows.map((c: any) => {
    ...
    // 3. Obtener empleados activos asignados a esta sucursal
    ...
    const empleadas = empleadosRows.map((e: any) => {
    ...
    console.error("[GET /api/pos/inicio Error]:", error);
    return NextResponse.json(
      { error: "Error al cargar datos del terminal POS", details: error.message },
      { status: 500 }
    );
  }
}
